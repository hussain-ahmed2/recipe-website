<x-layout>
    ...
    <div class="w-full max-w-7xl mx-auto px-6 my-14">
        ...
        <form class="flex flex-col gap-4" action="/recipes/{{ $recipe->id }}" method="POST"
            ...
            <div>
                ...
                <button type="button" onclick="addIngredient()"
                    class="mt-2 bg-cyan-500 text-white px-4 py-2 rounded ml-auto block hover:bg-cyan-600">Add
                    Ingredient</button>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <x-forms.input-field name="prepTimeMinutes" type="number" label="Prep Time (minutes)"
                    value="{{ old('prepTimeMinutes', $recipe->prepTimeMinutes) }}" />
                <x-forms.input-field name="cookTimeMinutes" type="number" label="Cook Time (minutes)"
                    value="{{ old('cookTimeMinutes', $recipe->cookTimeMinutes) }}" />
            </div>
            ...
            <x-forms.input-field name="caloriesPerServing" type="number" label="Calories per Serving"
                value="{{ old('caloriesPerServing', $recipe->caloriesPerServing) }}" />

            {{-- Image Upload --}}
            <div>
                <label class="block font-medium mb-1">Current Image</label>
                <img src="{{ Str::startsWith($recipe->image, 'http') ? $recipe->image : Vite::asset($recipe->image) }}"
                    alt="Recipe Image" class="w-32 h-32 rounded-md object-cover mb-2">
                <x-forms.input-field name="image" type="file" label="Upload New Image (optional)"
                    :required="false" />
            </div>
                ...
                <select name="category_id"
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}"
                            {{ $recipe->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="my-10">
            <button id="modal-open-btn" class="btn-danger">Delete Recipe</button>
            <div id="modal" class="fixed top-0 left-0 w-full h-full z-10 bg-black/30 grid place-content-center scale-0 transition-transform duration-300">
                <div class="border border-gray-300 rounded-3xl bg-neutral-50 p-14 max-w-3xl mx-auto ">
                    <p class="text-center mb-8">Are you sure you want to delete this recipe? This action cannot be undone.</p>
                    <div class="flex justify-center gap-12">
                        <button id="modal-close-btn" class="btn">Cancel</button>
